...
else
{
    ...
    $javascriptCode = '
    	...
    	$("#quelle").change(function () {
        	if($(this).val().length > 0) {
                window.location.replace("'. ADMIDIO_URL . '/adm_plugins/'.$plugin_folder.'/copy.php?full_screen='.$getFullScreen.'&target_userid='.$getTargetUserid.'&source_userid="+$(this).val());
            }
        });
        ...
        $("#ziel").change(function () {
            if($(this).val().length > 0) {
                window.location.replace("'. ADMIDIO_URL . '/adm_plugins/'.$plugin_folder.'/copy.php?full_screen='.$getFullScreen.'&source_userid='.$getSourceUserid.'&target_userid="+$(this).val());
            }
        });
        ...
        $("input[type=checkbox].targetlist_checkbox").click(function(){
			...
			if(sourcecheckbox.size() == 1) {
				var row_id = sourcecheckbox.attr("id");
             	...
             	$.post("'. ADMIDIO_URL . '/adm_plugins/'.$plugin_folder.'/copy.php?mode=assign&full_screen='.$getFullScreen.'&source_usfid="+source_usfid+"&target_usfid="+target_usfid+"&target_userid='.$getTargetUserid.'&source_userid='.$getSourceUserid.'",
                	function(data){
    ...
    $copyMenu->addItem('menu_item_back', ADMIDIO_URL .'/adm_plugins/'.$plugin_folder.'/menue.php?show_option=copy', $gL10n->get('SYS_BACK'), 'back.png');

    if($getFullScreen == true)
    {
    	$copyMenu->addItem('menu_item_normal_picture', ADMIDIO_URL . '/adm_plugins/'.$plugin_folder.'/copy.php?source_userid='.$getSourceUserid.'&amp;target_userid='.$getTargetUserid.'&amp;full_screen=0',
                $gL10n->get('SYS_NORMAL_PICTURE'), 'arrow_in.png');
    }
    else
    {
        $copyMenu->addItem('menu_item_full_screen', ADMIDIO_URL . '/adm_plugins/'.$plugin_folder.'/copy.php?source_userid='.$getSourceUserid.'&amp;target_userid='.$getTargetUserid.'&amp;full_screen=1',
