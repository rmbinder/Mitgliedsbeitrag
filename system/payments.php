...
try {
    ...
    if (strpos($gNavigation->getUrl(), 'payments.php') === false) {
        ...
        $_SESSION['pMembershipFee']['payments_rol_sel'] = array();
    }
    if (isset($_GET['payments_roleselection'])) {
        $_SESSION['pMembershipFee']['payments_rol_sel'] = array_filter($_GET['payments_roleselection']); // array_filter: löschen leerer Einträge
    }
    ...
    if ($getMode == 'assign_date') {
        ...
        if (count($_SESSION['pMembershipFee']['payments_rol_sel']) > 0) {
       ...
       WHERE fee.usd_value IS NOT NULL ';
        ...
        if ($getMode == 'prepare') {
            ...
            $page = PagePresenter::withHtmlIDAndHeadline('plg-membershipfee-payments');
            ...
            $selectBoxEntries = array(
                ...
                '1' => $gL10n->get('PLG_MEMBERSHIPFEE_WITH_PAID'),
                '2' => $gL10n->get('PLG_MEMBERSHIPFEE_WITHOUT_PAID')
            );
            ...
            $form->addSelectBox('payments_roleselection', '', $selectBoxEntriesBeitragsrollen, array(
                'defaultValue' => $_SESSION['pMembershipFee']['payments_rol_sel'],
                'showContextDependentFirstEntry' => false,
            ...
            $form->addInput('datum', $gL10n->get('PLG_MEMBERSHIPFEE_PAID_ON'), $datum, array(
                'type' => 'date',
                'helpTextId' => 'PLG_MEMBERSHIPFEE_PAID_DESC'
            ));

            $form->addButton('btn_save_date', $gL10n->get('PLG_MEMBERSHIPFEE_SAVE_DATE'), array(
                'icon' => 'bi-check-lg',
                'link' => 'javascript:assign_date()',
                'class' => 'btn-primary'
            ));

            $form->addButton('btn_delete_date', $gL10n->get('PLG_MEMBERSHIPFEE_DELETE_DATE'), array(
                'icon' => 'bi-trash',
                'link' => 'javascript:delete_date()',
                'class' => 'btn-primary'
            ));
        ...
        function assign_date() {
            var datum = $("#datum").val();
            $.post("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/payments.php', array(
                'mode' => 'assign_date'
            )) . '&datum_neu=" + datum,
                function(data){
        ...
        $("input[type=checkbox].memlist_checkbox").click(function(e){
            ...
            $.post("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/payments.php', array(
                'mode' => 'prepare'
            )) . '&checked=" + member_checked + "&usr_id=" + userid,
                function(data){
                    // check if error occurs
                    if(data !== "success") {
                        alert(data);
        ...
        ';
            ...
            $table = new DataTables($page, 'table_payments');
            ...
            $data['headers'][] = '<a class="icon-text-link" href="javascript:select_all()">' . $gL10n->get('SYS_ALL') . '</a>';
            ...
            $htmlTable = $smarty->fetch('../templates/payments.plugin.membershipfee.tpl');
