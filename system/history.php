	...
	elseif ($getMode === 'html')
	{
        ...
        $page->addJavascript('
            $("#menu_item_lists_print_view").click(function() {
                window.open("'.SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER .'/system/history.php', array(
        ...
        if ($getExportAndFilter)
        {
            ...
            $page->addPageFunctionsMenuItem('menu_item_lists_xlsx', $gL10n->get('SYS_MICROSOFT_EXCEL').' (XLSX)',
                SecurityUtils::encodeUrl(ADMIDIO_URL.FOLDER_PLUGINS . PLUGIN_FOLDER .'/system/history.php', array(
                    'filter_date_from'  => $getDateFrom,
                    'filter_date_to'    => $getDateTo,              
                    'filter_last_name'  => $getLastName,
                    'filter_first_name' => $getFirstName,
        ...
        if ($getExportAndFilter)
        {    
            $form = new HtmlForm('navbar_filter_form', SecurityUtils::encodeUrl(ADMIDIO_URL.FOLDER_PLUGINS . PLUGIN_FOLDER .'/system/history.php'),  $page, array('type' => 'navbar', 'setFocus' => false));
