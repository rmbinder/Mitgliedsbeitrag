 ...
 *                    1 - Nur Benutzer anzeigen, bei denen eien E-Mail vorhanden ist
 *                    2 - Nur Benutzer anzeigen, bei denen keine E-Mail vorhanden ist
...
if ($getMode == 'mail' || $getMode == 'export') {
    ...
    if ($getMode == 'prepare') {
        ...
        $page->addJavascript('
            function billexport(){ 
                $.post("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/bill.php', array(
            'mode' => 'export'
        )) . '",
                    function(data){
                        ...
                        if(data == "marker_empty") {
                            ...
                            window.location.href = "' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/bill_export.php') . '" ;
                        }
                        return true;
                    }
                );
            ...
            $.post("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/bill.php', array(
            'mode' => 'prepare',
            'mem_show_choice' => $getMembersShow
        )) . '" ,
                function(data){
                    // check if error occurs
                    if(data == "success") {
                        window.location.replace("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/bill.php') . '");
                    }
                    else {
                        alert(data);
                        return false;
       ...
       // Anzeige abhaengig vom gewaehlten Filter
        $("#mem_show").change(function () {
            if($(this).val().length > 0) {
                window.location.replace("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/bill.php') . '?mem_show_choice=" + $(this).val());
            }
        });
        ...
        $form = new HtmlForm('bill_filter_form', '', $page, array(
            'type' => 'navbar',
            'setFocus' => false
        ));
        ...
        $selectBoxEntries = array(
            ...
            '1' => $gL10n->get('PLG_MEMBERSHIPFEE_WITH_EMAIL'),
            '2' => $gL10n->get('PLG_MEMBERSHIPFEE_WITHOUT_EMAIL')
        );
        ...
        foreach ($membersList as $member => $memberData) {
            foreach ($memberData as $usfId => $dummy) {
                ...
                if ($gProfileFields->getPropertyById($usfId, 'usf_type') === 'CHECKBOX' || $gProfileFields->getPropertyById($usfId, 'usf_name_intern') === 'GENDER' || $gProfileFields->getPropertyById($usfId, 'usf_type') === 'EMAIL' || $usfId === (int) $gProfileFields->getProperty('DEBTOR_EMAIL', 'usf_id')) {
                    $columnAlign[] = 'center';
                    ...
                    if (strlen($content) > 0) {
                        if ($gSettingsManager->getInt('mail_module_enabled') != 1) {
                            ...
                            $mail_link = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/message_write.php', array(
                                'user_uuid' => $user->getValue('usr_uuid'),
                                'usf_uuid' => $userField->getValue('usf_uuid')
                            ));
                        }
                        $columnValues[] = '<a class="admidio-icon-link" href="' . $mail_link . '"><i class="bi bi-envelope" title="' . $gL10n->get('SYS_SEND_EMAIL_TO', array(
                            $content
                        )) . '"></i>';
                    } else {
                        $columnValues[] = '';
