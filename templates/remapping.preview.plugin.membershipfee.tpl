<form {foreach $attributes as $attribute}
    ...
    <div class="table-responsive">
        <table id="table_preview_remapping" class="{$classTable}" style="max-width: 100%;">
            <thead>
                <tr>
            ...
            <tbody>          
                {if count($rows) eq 0}
                    <tr>
                        <td colspan="{count($headers)}" style="text-align: center;">{$l10n->get('PLG_MEMBERSHIPFEE_REMAPPING_NO_ASSIGN')}</td>
                       
                        {* Wenn die nächsten 6 Data-Zeilen nicht vorhanden sind, wird seltsamerweise nachfolgender Fehler ausgeworfen: *}                  
    ...
    {if count($rows) !== 0}
        <p>{$l10n->get('PLG_MEMBERSHIPFEE_REMAPPING_PREVIEW')}</p>  
